...
public class EditCompleteRoomController {
	...
	public String init(ModelMap model, HttpSession session, RedirectAttributes attribute) {
		logger.info("EditCompleteRoomController-init");

		model = buildingService.reloadModel(model, session);

		attribute.addFlashAttribute("buildingId", model.get("buildingId"));
		attribute.addFlashAttribute("roomId", model.get("roomId"));
